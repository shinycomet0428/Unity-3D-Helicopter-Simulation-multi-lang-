...
void Model::create_shader(std::string vertex_shader_file_path, std::string fragment_shader_file_path)
{
    ...
    this->shader = new Shader(vertex_shader_file_path.c_str(), fragment_shader_file_path.c_str());
}
...
{      
    ...
    this->shader->setVec3("mainrotor", glm::vec3(0.0f));
    this->shader->setVec3("tailrotor", glm::vec3(0.0f));   
}
...
{
    this->model = glm::rotate(this->model, angle, rotation);
}
